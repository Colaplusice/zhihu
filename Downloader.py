...
class Beginer:
    def __init__(self, Mongo_cache=None, downloader=ZhihuAccount(),
                 begin_url='https://www.zhihu.com/people/fan-jia-liang-57/following',
                 throttle=None,html_mon_cache=None):
        ...
        for (k, v) in users.items():
            ...
            try:
                if k !='fan-jia-liang-57':
                    values = {'昵称': v['name'], '标题': v['headline'] if not re.findall('<a.*?/a>', v['headline']) else '',
                              '性别': '女' if v['gender'] == 0 else '男',
                              '回答数目': v['answerCount'] if v.get('answerCount')else '', '关注者': v['followerCount'], '文章': v['articlesCount'],
...
if __name__ == '__main__':
    ...
    password = 'your pass'
    begin=Beginer(Mongo_cache=Mongo_cache(),downloader=account,begin_url=begin_url,throttle=Throttle(delay=5)
