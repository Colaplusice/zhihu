...
class Mongo_cache:
    ...
    def __init__(self, client=None, expired_time=timedelta(days=30)):
        self.client = (
            mongo_client.MongoClient("localhost", 27017) if client is None else client
        )
        # if client.cache is not Non
        self.db = self.client.zhihu
        # 创建时间戳index  过期后mongodb会自动删除

        self.db.message.create_index(
            "timestamp", expireAfterSeconds=expired_time.total_seconds()
        )
...
class Mongo_html_cache:
    ...
    def __init__(self, client=None, expired_time=timedelta(days=30)):
        ...
        self.db.html_message.create_index(
            "timestamp", expireAfterSeconds=expired_time.total_seconds()
        )
        self.expired_time = expired_time
